...
export function UpdateNotification() {
  ...
  useEffect(() => {
    const unsubChecking = tauri.onUpdateChecking(() => {
      setStatus({ type: 'checking' });
      setVisible(true);
    });

    const unsubAvailable = tauri.onUpdateAvailable((version: string) => {
      setStatus({ type: 'available', version });
      setVisible(true);
    });

    const unsubNotAvailable = tauri.onUpdateNotAvailable(() => {
      setStatus({ type: 'none' });
      setVisible(false);
    });

    const unsubProgress = tauri.onUpdateProgress((percent: number) => {
      setStatus({ type: 'downloading', percent });
      setVisible(true);
    });

    const unsubReady = tauri.onUpdateReady((version: string) => {
      setStatus({ type: 'ready', version });
      setVisible(true);
    });

    const unsubError = tauri.onUpdateError((message: string) => {
