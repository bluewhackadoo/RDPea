...
export function SessionView() {
  ...
  useEffect(() => {
    ...
    console.log('[SessionView] Setting up IPC listeners for connection:', connectionId);

    const unsubFrame = tauri.onFrame((id, rects) => {
      console.log('[SessionView] Frame received:', id, rects.length, 'rects');
      if (id === connectionId) renderFrame(rects);
    });
    const unsubAudio = tauri.onAudio((id, audioData) => {
      if (id === connectionId) playAudio(audioData);
    });
    const unsubConnected = tauri.onConnected((id, info) => {
      console.log('[SessionView] Connected:', id, info);
      if (id === connectionId) {
    ...
    const unsubDisconnected = tauri.onDisconnected((id) => {
      console.log('[SessionView] Disconnected:', id);
      if (id === connectionId) { setIsConnected(false); setIsConnecting(false); }
    });
    const unsubError = tauri.onError((id, msg) => {
      console.log('[SessionView] Error:', id, msg);
      if (id === connectionId) { setErrorMsg(msg); setIsConnecting(false); }
    });

    const unsubDebug = tauri.onDebugLog((id, msg) => {
      console.log('[SessionView] Debug log:', id, msg);
      if (id === connectionId) {
        setDebugLogs(prev => {
          const next = [...prev, msg];
    ...
    const unsubDebugGlobal = tauri.onDebugGlobal((enabled) => {
      console.log('[SessionView] Debug global:', enabled);
      setDebugMode(enabled);
      setDebugOpen(enabled);
      if (connectionId) tauri.setDebug(connectionId, enabled);
    });

    // Check initial status
    tauri.getStatus(connectionId).then((status) => {
      console.log('[SessionView] Initial status:', status);
      setIsConnected(status);
    });

    // Check if global debug was already enabled before this window opened
    tauri.getDebugGlobal().then((enabled) => {
      console.log('[SessionView] Initial debug global:', enabled);
      if (enabled) {
        setDebugMode(true);
        setDebugOpen(true);
        if (connectionId) tauri.setDebug(connectionId, true);
      }
    });
  ...
  useEffect(() => {
    ...
    const handleKey = (e: globalThis.KeyboardEvent) => {
      ...
      tauri.sendKeyboard(connectionId, type, mapping.code, mapping.extended);
    };
    ...
    const handleBlur = () => {
      pressedModifiersRef.current.forEach((code) => {
        ...
        if (mapping) {
          tauri.sendKeyboard(connectionId, 'keyup', mapping.code, mapping.extended);
        }
      });
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    tauri.sendMouse(connectionId, 'move', x, y);
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    ensureAudioCtx(); // Resume AudioContext from user gesture
    const { x, y } = getCanvasCoords(e);
    tauri.sendMouse(connectionId, 'down', x, y, buttonName(e.button));
  }, [connectionId, isConnected, getCanvasCoords, ensureAudioCtx]);

  const handleMouseUp = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    if (!connectionId || !isConnected) return;
    const { x, y } = getCanvasCoords(e);
    tauri.sendMouse(connectionId, 'up', x, y, buttonName(e.button));
  }, [connectionId, isConnected, getCanvasCoords]);
  ...
  const handleWheel = useCallback((e: React.WheelEvent<HTMLCanvasElement>) => {
    ...
    tauri.sendMouse(connectionId, 'wheel', x, y, undefined, delta);
  }, [connectionId, isConnected, getCanvasCoords]);
  ...
  const handleFullscreen = () => tauri.maximize();
  ...
  const handleReconnect = async () => {
    ...
    try {
      const connections = await tauri.loadConnections();
      const conn = connections.find((c: any) => c.id === connectionId);
      if (conn) {
        await tauri.connect(conn);
      }
    } catch {
